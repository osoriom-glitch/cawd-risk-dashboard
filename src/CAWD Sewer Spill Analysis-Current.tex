...
approximately \$525K under the optimized allocation.

\begin{table}[H]
\centering
\caption{Tornado diagram summary (SA3). Base expected annual cost
  $\approx$ \$525K (optimized allocation, seeded MC, 4{,}000 runs).
...
Spill volume (min $\to$ max) & Cat.\ floors
  & $\sim$\$110K & $\sim$\$8.2M & $\sim$\$8.1M \\
Failure rate $\lambda$ ($\times 0.5 \to \times 2$) & $\lambda \times 0.5$
  & $\sim$\$260K & $\sim$\$1.05M & $\sim$\$0.79M \\
...
annual cost falls to approximately \$110K---a reduction of
\$415K from the \$525K base. When all spills occur at maximum volume
(category ceilings), expected annual cost rises to approximately
\$8.2M, driven by the single-spill contribution of a Cat.~1 event at
the 145{,}000-gallon ceiling: a single such spill costs
$\$100{,}000 + \$10 \times 145{,}000 = \$1.55$M, and the cost model
is linear in volume above the fixed charge. The total swing exceeds
\$8.1M, confirming that the upper tail of the volume distribution
dominates the expected-cost calculation. This has two implications
...
reduces expected annual cost to approximately \$260K. Scaling by 2.0
(reflecting a pessimistic scenario of accelerated aging or
under-investment) raises it to approximately \$1.05M. The total
swing of approximately \$0.79M reflects the fact that expected cost
...
$E[U]$ falls sharply, and the optimizer under this setting would
be expected to shift even more of the budget toward prevention. At
$\rho = \$100$M (near-risk-neutral, $\rho \times 10$), $E[U]$
approaches $-E[C]/\rho$, and the objective reduces approximately to
expected-cost minimization. The calibrated value $\rho = \$10$M
...
\begin{figure}[H]
  ...
  \caption{Tornado diagram (SA3). Each bar shows the deviation of
    expected annual cost from the base scenario (\$525K) when one
    input is set to its low (green, cost-reducing) or high (red,
    cost-increasing) extreme. Spill volume dominates with a swing
    exceeding \$8M; failure rate uncertainty contributes a
    \$0.79M swing. Risk tolerance $\rho$ does not affect
    expected cost and is omitted from the cost axis.}
  \label{fig:sa3}
\end{figure}
