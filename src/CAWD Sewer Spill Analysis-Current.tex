...
\author{Annabelle Jayadinata, Maura Osorio, Khang Do, Ramesh Manian}
...
\paragraph{Results.}
Figure~\ref{fig:sa1} shows the budget-sweep results. Below \$225K,
the optimizer allocates exclusively to Preventive Maintenance ($I_1$),
which carries the highest per-dollar effectiveness for blockage
reduction ($\alpha_{B,I_1} = 0.040$, the dominant failure mode at
84\% of historical spills). At \$225K, Risk-Based Targeting ($I_4$)
first clears the entry threshold---holding $\geq$0.5 units for three
consecutive budget steps---marking the diversification point beyond
which the optimizer splits resources across multiple levers.

Above \$225K, the allocation is mixed throughout the sweep. $I_1$
remains the primary lever at most budget levels, but $I_3$ (Execution
Quality) and $I_4$ (Risk-Based Targeting) receive meaningful
allocations alongside it as the budget grows. At the \$1M board
proposal, the optimizer distributes investment across $I_1$, $I_3$,
and $I_4$, reflecting complementary blockage-mitigation pathways:
maintenance reduces recurrence, targeting concentrates effort on
high-risk pipe segments, and execution quality reduces the probability
of maintenance failures. Because the \$1M proposal sits well above
the \$225K diversification threshold, the board's investment plan
should incorporate multiple levers rather than concentrate
exclusively in preventive maintenance.

Expected utility $E[U(-C)]$ in Figure~\ref{fig:sa1} (right panel) is
shown as both a raw Monte Carlo trace and a smoothed overlay. The raw
series remains locally non-monotonic due to finite-sample variability,
but the 9-point moving-average line makes the directional pattern
clearer: utility generally improves as budget increases. To reduce
point-level noise, SA1 is evaluated with 1{,}600 Monte Carlo runs per
budget point. Accordingly, SA1 is interpreted using both the
diversification threshold (\$225K) and the smoothed utility trend,
rather than requiring strict monotonicity in every adjacent point.
...
\begin{figure}[H]
  ...
  \caption{Budget-threshold sensitivity analysis (SA1). Left: investment
    allocation by budget level. Below \$225K the optimizer concentrates
    entirely in Preventive Maintenance ($I_1$, blue); the yellow dashed
    line marks the \$225K diversification threshold where Risk-Based
    Targeting ($I_4$, purple) first enters the solution. Above \$225K
    allocation is mixed across $I_1$, $I_3$, and $I_4$. The black
    dashed line marks the \$1M board proposal. Right: expected utility
    $E[U(-C)]$ by budget with two traces: raw MC estimates (yellow)
    and a 9-point moving-average overlay (blue) to reveal the
    directional trend. Raw point-level variation remains non-monotonic.
    Seeded MC, 1{,}600 runs per point (SA1).}
  \label{fig:sa1}
\end{figure}
...
under two scenarios: unconstrained and with $I_1$ capped at \$300K
(3 units).
...
\label{tab:sa2-cap}
\begin{tabular}{lccccr r}
\toprule
\textbf{Cap on $I_1$} & \boldmath$I_1$ & \boldmath$I_2$ &
  \boldmath$I_3$ & \boldmath$I_4$ &
  \textbf{Unspent} & \textbf{$E[U]$} \\
\midrule
None (unconstrained) & 0.50 & 0.00 & 0.00 & 0.25 & 4.75\,u & $-$0.070 \\
3.0\,u cap (\$300K)  & 3.00 & 0.50 & 1.50 & 0.50 & 0.00\,u & $-$0.062 \\
\bottomrule
\end{tabular}
\end{table*}

In the unconstrained scenario, the greedy optimizer deploys only
\$75K of the \$550K budget (0.5\,u of $I_1$ and 0.25\,u of $I_4$),
leaving \$475K unallocated. Each subsequent \$25K step fails to
produce a utility improvement distinguishable from Monte Carlo
estimation noise at 1{,}500 samples per evaluation, so the optimizer
halts early. This reflects a modeling boundary: at current
failure-rate parameters, the per-step marginal utility gain is below
the noise floor, and the greedy algorithm cannot justify further
deployment from the simulation evidence alone.

When $I_1$ is capped at \$300K (3 units), a qualitatively different
outcome emerges: the optimizer deploys the \emph{entire} \$550K
budget. With the dominant lever restricted, the model exhausts the
remaining capacity across $I_2$ (Workforce Capacity, 0.5\,u),
$I_3$ (Execution Quality, 1.5\,u), and $I_4$ (Risk-Based Targeting,
0.5\,u). The priority order among secondary levers follows the
effectiveness matrix: $I_3$ receives the largest secondary allocation
because it carries the highest effectiveness coefficients for the
pump and operational failure modes once the dominant blockage levers
are saturated. Expected utility improves from $-0.070$ in the
unconstrained run to $-0.062$ under the cap---a counter-intuitive
result explained by the constraint forcing full budget deployment
that the unconstrained optimizer defers due to noisy per-step signals.
...
\begin{figure}[H]
  ...
  \caption{Maintenance-cap sensitivity (SA2). Left: unconstrained
    allocation at \$550K budget; optimizer deploys only \$75K
    ($I_1$=0.5\,u, $I_4$=0.25\,u), leaving \$475K unspent.
    Right: allocation when $I_1$ is capped at \$300K (3.0\,u);
    full \$550K deployed across all four levers ($I_1$=3.0,
    $I_2$=0.5, $I_3$=1.5, $I_4$=0.5\,u). Dashed line marks the
    cap level. Red bar indicates the constrained lever.}
  \label{fig:sa2}
\end{figure}

These results have a direct operational interpretation. The
unconstrained optimizer's early termination at \$75K reflects the
limit of the per-step MC signal at 1{,}500 samples: the full \$550K
budget is genuinely beneficial, but the greedy algorithm cannot
detect the incremental gain above the noise floor. The cap scenario
demonstrates that once a binding constraint forces exploration of
secondary levers, Execution Quality ($I_3$) and Risk-Based Targeting
($I_4$) together absorb the entire remaining capacity. The
blockage-reduction gap created by the maintenance cap is therefore
partially bridged by the secondary lever mix, though no combination
of $I_2$, $I_3$, and $I_4$ fully replicates the effectiveness of
unrestricted preventive maintenance on the dominant failure mode.
